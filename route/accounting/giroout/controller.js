...
const controller = {
  //   REST-SELECT
  getGirooutOptions_get: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { bankAccount: [], girooutBusinesspartner: [], girooutDepartment: [], project: [] }, label: 'Berhasil' };
  ...
  //   REST-INSERT
  giroout_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { lastId: '', giroout_code: '', general_journal_id: '' }, label: 'Data entered successfully' };
    // validation
    ...
    var data = {
      reference_code: '',
      general_journal_type_id: '13', //important code
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        var codeData = {
          special_code_id: "GIROUT",
          table: "giroout",
          column_id: "giroout_id",
          column_code: "giroout_code",
        }

        var myfireStr = `SELECT concat(t1.special_code, DATE_FORMAT(now(), t1.date_format), (SELECT count(t2.${codeData.column_id}) + 1 + t1.start_number FROM ${CUS_DB}.${codeData.table} t2 WHERE t2.${codeData.column_code} LIKE concat(t1.special_code, DATE_FORMAT(now(), t1.date_format), "%"))) AS "code", (SELECT generalledger_period_id FROM ${CUS_DB}.generalledger_period WHERE is_use = 1 AND is_active = 1 ORDER BY update_datetime DESC LIMIT 1) as "generalledger_period_id", (SELECT account_id FROM ${CUS_DB}.account_linked WHERE is_use = 1 AND is_active = 1 AND account_link_id = 15 LIMIT 1) as "account_id", (${querystrCode}) as "journal_code" FROM ${CUS_DB}.special_code t1 WHERE t1.is_use = 1 AND t1.is_active = 1 AND t1.special_code_id = "${codeData.special_code_id}" LIMIT 1`; // --account_link_id 15

        //   -SELECT-SPECIAL_CODE   -SELECT-GIROOUT
        var query = conn.query(myfireStr, function (err, rows) {
          ...
          if (rows[0]['code'] && rows[0]['generalledger_period_id'] && rows[0]['account_id']) {
            ...
            giroout_code = rows[0]['code'];
            data.reference_code = giroout_code;
                    ...
                    var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                      ...
                      tsservice.insertData(data2, function (value) {
                        ...
                        var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                          ...
                          continues(req, giroout_code, general_journal_id, conn);
    ...
    function continues(req, giroout_code, general_journal_id, conn) {

      const CUS_DB = req.body.company_db;

      var data = {
        giroout_code: giroout_code,
      ...
      tsservice.insertData(data, function (value) {
        //   -INSERT-GIROOUT
        var query = conn.query(`INSERT INTO ${CUS_DB}.giroout` + value, function (err, rows) {
          ...
          row.data.giroout_code = giroout_code;
  ...
  //   REST-UPDATE
  giroout_put: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: {}, label: 'Data updated successfully' };
    // validation
    req.assert('giroout_id', 'Journal Id is required');
    req.assert('giroout_code', 'Journal Code/No is required');
    req.assert('transaction_date', 'Transaction Date is required');
    req.assert('efective_begin', 'Transaction Date is required');
    req.assert('efective_end', 'Transaction Date is required');
    req.assert('account_id', 'Generalledger Period Id is required');
    ...
    // update data table giroout
    var data = {
      giroout_code: req.body.giroout_code,
    ...
    var data2 = {
      reference_code: req.body.giroout_code,
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        var myfireStr = `SELECT account_id FROM ${CUS_DB}.account_linked WHERE is_use = 1 AND is_active = 1 AND account_link_id = 15 LIMIT 1`; // --account_link_id 15
    ...
    function continues(req, conn) {
      ...
      tsservice.updateData(data, function (value) {
        //   -UPDATE-GIROOUT
        var query = conn.query(`UPDATE ${CUS_DB}.giroout SET ${value} WHERE giroout_id =${req.body.giroout_id} `, function (err, rows) {
  ...
  //   REST-SELECT
  girooutSearch_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { giroout: [] }, label: 'Data selected successfully' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.*, t1.status_id as "old_status_id" , t2.generalledger_period_id, t2.transaction_date FROM ${CUS_DB}.giroout t1 INNER JOIN ${CUS_DB}.general_journal t2 ON t1.general_journal_id = t2.general_journal_id WHERE t1.is_use = 1 AND t1.is_active = 1`;
      //   -SELECT-GIROOUT   -JOIN-GENERAL_JOURNAL
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
