...
const controller = {
  //   REST-SELECT
  getReconciliation_adjOptions_get: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { bankAccount: [] }, label: 'Berhasil' };
  ...
  //   REST-INSERT
  reconciliation_adj_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { lastId: '', reconciliation_adj_code: '', general_journal_id: '' }, label: 'Data entered successfully' };
    // validation
    ...
    req.assert('debit', 'Deparment Id is required');
    req.assert('credit', 'Deparment Id is required');
      ...
      conn.beginTransaction(function (err) {
        ...
        var query = conn.query(myfireStr, function (err, rows) {
          ...
          if (rows[0]['code'] && rows[0]['generalledger_period_id'] && rows[0]['account_id']) {
            ...
            reconciliation_adj_code = rows[0]['code'];
            data.reference_code = reconciliation_adj_code;
            ...
            if (req.body.debit == 0 || req.body.credit == 0) {
              if (req.body.debit == 0) {
                data2.debit = req.body.credit;
                creditParam = req.body.credit;
                //if debit - credit = < 0 thats mean debit to reconciliation account
              } else {
                data2.credit = req.body.debit;
                debitParam = req.body.debit;
                        ...
                        var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                          ...
                          tsservice.insertData(data2, function (value) {
                            ...
                            var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                              ...
                              continues(req, reconciliation_adj_code, general_journal_id, conn);
              ...
              row.success = true;
              row.label = 'Make sure that one of debit or credit is 0';
              conn.rollback(function () {
                 res.send(row); return;
              });
            }
    ...
    function continues(req, reconciliation_adj_code, general_journal_id, conn) {

      const CUS_DB = req.body.company_db;
      
      var data = {
        reconciliation_adj_code: reconciliation_adj_code,
      ...
      tsservice.insertData(data, function (value) {
        //   -INSERT-RECONCILIATION_ADJ
        var query = conn.query(`INSERT INTO ${CUS_DB}.reconciliation_adj` + value, function (err, rows) {
          ...
          row.data.reconciliation_adj_code = reconciliation_adj_code;
  ...
  //   REST-UPDATE
  reconciliation_adj_put: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: {}, label: 'Data updated successfully' };
    // validation
    req.assert('reconciliation_adj_id', 'Journal Id is required');
    req.assert('reconciliation_adj_code', 'Journal Code/No is required');
    req.assert('transaction_date', 'Transaction Date is required');
    req.assert('account_id', 'Generalledger Period Id is required');
    ...
    // update data table reconciliation_adj
    var data = {
      reconciliation_adj_code: req.body.reconciliation_adj_code,
    ...
    var data2 = {
      reference_code: req.body.reconciliation_adj_code,
      general_journal_type_id: '8', //important code
      description: 'Generated',
      transaction_date: tsservice.mysqlDate(req.body.transaction_date),
      update_by: req.body.update_by,
    ...
    req.getConnection(function (err, conn) {
      ...
      tsservice.updateData(data, function (value) {
        //   -UPDATE-RECONCILIATION_ADJ
        var query = conn.query(`UPDATE ${CUS_DB}.reconciliation_adj SET ${value} WHERE reconciliation_adj_id =${req.body.reconciliation_adj_id} `, function (err, rows) {
  ...
  //   REST-INSERT
  reconciliation_adjSearch_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { reconciliation_adj: [] }, label: 'Data selected successfully' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.* , t2.generalledger_period_id, t2.transaction_date FROM ${CUS_DB}.reconciliation_adj t1 INNER JOIN ${CUS_DB}.general_journal t2 ON t1.general_journal_id = t2.general_journal_id WHERE t1.is_use = 1 AND t1.is_active = 1`;

      //   -SELECT-RECONCILIATION_ADJ   -JOIN-GENERAL_JOURNAL
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
        ...
        row.data.reconciliation_adj = rows;
         res.send(row); return;
      });
