...
const controller = {
  //   REST-SELECT
  getInventoryoutList_get: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { InventoryoutList: [] }, label: 'Berhasil' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT *, DATE_FORMAT(transaction_date, "%d %M %Y") AS "show_date" FROM ${CUS_DB}.inventory_stock_change WHERE is_use = 1 AND is_active =1 AND methode_id = 2`;
      ...
      var query = conn.query(myfireStr, function (err, rows) {
        ...
        row.data.InventoryoutList = rows;
         res.send(row); return;
  ...
  //   REST-SELECT
  inventoryouts_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.*, t1.debit - t1.credit AS "quantity" , t2.warehouse, t3.inventory_code, t3.name as "inventory_name", t3.uom1 FROM ${CUS_DB}.inventoryledger t1 INNER JOIN ${CUS_DB}.warehouse t2 ON t1.warehouse_id = t2.warehouse_id INNER JOIN ${CUS_DB}.inventory t3 ON t1.inventory_id = t3.inventory_id WHERE t1.credit <> 0 AND t1.warehouse_id <> 1 AND t1.inventoryledger_link_id = "${req.body.inventoryledger_link_id}"`;
  ...
  //   REST-INSERT
  inventoryout_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    ...
    req.assert('isimported', 'isimported is required');
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        if (req.body.inventory_stock_change_id) {
          ...
          var myfireStr = `SELECT special_code as "code", (SELECT generalledger_period_id FROM ${CUS_DB}.generalledger_period WHERE generalledger_status_id = 1 AND is_active =1 AND is_use =1 limit 1) as "generalledger_period_id" FROM ${CUS_DB}.special_code WHERE is_use = 1 AND is_active = 1 AND special_code_id = "INVOUT" LIMIT 1`;
          ...
          var query = conn.query(myfireStr, function (err, rows) {
            ...
            if (rows[0]['code'] && rows[0]['generalledger_period_id']) {
              ...
              var data = {
                ...
                general_journal_type_id: '17', //important code
              ...
              tsservice.insertData(data, function (value) {
                ...
                var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                  ...
                  var myfireStr = `SELECT special_code as "code" FROM ${CUS_DB}.special_code WHERE is_use = 1 AND is_active = 1 AND special_code_id = "INVOUT" LIMIT 1`;
                  ...
                  var query = conn.query(myfireStr, function (err, rows) {
                    ...
                    if (rows[0]['code']) {
                      ...
                      var data = {
                        ...
                        inventoryledger_link_type_id: 7, //important code
      ...
      return new Promise(function (resolve, reject) {
        ...
        async.forEach(req.body.detail, function (item, callback) {
          ...
          if (item.inventoryledger_id == '') {
            ...
            queryStr += '( 0, 0, ' + item.quantity + ', 0, "' + inventoryledger_link_id + '", "' + item.inventory_id + '", ' + item.hpp + ', ' + debitSum + ' , ' + item.rate + ', ' + item.warehouse_id + ', "' + item.currency_id + '", "' + tsservice.mysqlDate(item.expired_date) + '", ' + item.reminder_expired_date + ', ' + item.isfix_asset + ', ' + req.body.create_by + ', ' + req.body.update_by + ', "' + tsservice.mysqlDate(req.body.create_datetime) + '", "' + tsservice.mysqlDate(req.body.create_datetime) + '", 1, 1, 1 )';

            //THE DEBIT CREDIT AND WAREHOUSE IS CHANGE
            // queryStr += ', ( ' + item.quantity + ', 0, 0, 0, "' + inventoryledger_link_id + '", "' + item.inventory_id + '", ' + item.hpp + ', ' + debitSum + ' , ' + item.rate + ', 0, "' + item.currency_id + '", "' + tsservice.mysqlDate(item.expired_date) + '", ' + item.reminder_expired_date + ', ' + item.isfix_asset + ', ' + req.body.create_by + ', ' + req.body.update_by + ', "' + tsservice.mysqlDate(req.body.create_datetime) + '", "' + tsservice.mysqlDate(req.body.create_datetime) + '", 1, 1 )';
            ...
            var query = conn.query(myfireStr, function (err, rows) {
              ...
              var query = conn.query(myfireStr, function (err, rows) {
                ...
                async.forEach(req.body.detail, function (item, callback) {
                  ...
                  queryStr = '( ' + general_journal_id + ', ' + rows[0].account_id + ', 0, 0, ' + debitValue + ', 0, 1, 1, "' + tsservice.mysqlDate(req.body.create_datetime) + '", "' + tsservice.mysqlDate(req.body.create_datetime) + '", 1, 1 )';

                  queryStr += ', ( ' + general_journal_id + ', ' + req.body.account_id + ', ' + debitValue + ', 0, 0, 0, 1, 1, "' + tsservice.mysqlDate(req.body.create_datetime) + '", "' + tsservice.mysqlDate(req.body.create_datetime) + '", 1, 1 )';
