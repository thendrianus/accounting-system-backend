...
const controller = {
  //   REST-SELECT
  getInventoryadjList_get: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { InventoryadjList: [] }, label: 'Berhasil' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT *, DATE_FORMAT(transaction_date, "%d %M %Y") AS "show_date" FROM ${CUS_DB}.inventory_stock_change WHERE is_use = 1 AND is_active =1 AND methode_id = 3`;
      ...
      var query = conn.query(myfireStr, function (err, rows) {
        ...
        row.data.InventoryadjList = rows;
        res.send(row); return;
  ...
  //   REST-SELECT
  inventoryadjs_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
  ...
  //   REST-INSERT
  inventoryadj_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        if (req.body.inventory_stock_change_id) {
          ...
          var myfireStr = `SELECT special_code as "code", (SELECT generalledger_period_id FROM ${CUS_DB}.generalledger_period WHERE generalledger_status_id = 1 AND is_active =1 AND is_use =1 limit 1) as "generalledger_period_id" FROM ${CUS_DB}.special_code WHERE is_use = 1 AND is_active = 1 AND special_code_id = "INVREC" LIMIT 1`;
          ...
          var query = conn.query(myfireStr, function (err, rows) {
            ...
            if (rows[0]['code'] && rows[0]['generalledger_period_id']) {
              ...
              var data = {
                ...
                general_journal_type_id: '18', //important code
              ...
              tsservice.insertData(data, function (value) {
                ...
                var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                  ...
                  var myfireStr = `SELECT special_code as "code" FROM ${CUS_DB}.special_code WHERE is_use = 1 AND is_active = 1 AND special_code_id = "INVREC" LIMIT 1`;
                  ...
                  var query = conn.query(myfireStr, function (err, rows) {
                    ...
                    if (rows[0]['code']) {
                      ...
                      var data = {
                        ...
                        inventoryledger_link_type_id: 3, //important code
              ...
              var query = conn.query(myfireStr, function (err, rows) {
                ...
                async.forEach(req.body.detail, function (item, callback) {
                  ...
                  if (theValue == 0) {
                    ...
                    var query = conn.query(`INSERT INTO ${CUS_DB}.generalledger( general_journal_id, account_id, debit, debiteqv, credit, crediteqv, create_by, update_by, create_datetime, update_datetime, is_use, is_active ) VALUES ` + queryStr, function (err, rows) {
