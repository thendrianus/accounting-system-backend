...
const controller = {
  //   REST-SELECT
  getInventorybbList_get: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { InventorybbList: [] }, label: 'Berhasil' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT *, DATE_FORMAT(transaction_date, "%d %M %Y") AS "show_date" FROM ${CUS_DB}.inventory_stock_change WHERE is_use = 1 AND is_active =1 AND methode_id = 4`;
  ...
  //   REST-SELECT
  inventorybbs_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
  ...
  //   REST-INSERT
  inventorybb_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
            ...
            if (rows[0]['code'] && rows[0]['generalledger_period_id']) {
              ...
              tsservice.insertData(data, function (value) {
                ...
                var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                  ...
                  var myfireStr = `SELECT special_code as "code" FROM ${CUS_DB}.special_code WHERE is_use = 1 AND is_active = 1 AND special_code_id = "BEGINNINGINVENTORY" LIMIT 1`;
                  ...
                  var query = conn.query(myfireStr, function (err, rows) {
                    ...
                    if (rows[0]['code']) {
                      ...
                      var data = {
                        ...
                        inventoryledger_link_type_id: 1, //important code
                        description: 'Inventory Beginning Stock',
        ...
        async.forEach(req.body.detail, function (item, callback) {
          ...
          if (queryStr != "") {
            ...
            var query = conn.query(myfireStr, function (err, rows) {
              ...
              var myfireStr = `SELECT * FROM ${CUS_DB}.account_linked WHERE is_use = 1 AND is_active = 1 AND (account_link_id = 2 OR account_link_id = 1) ORDER BY account_link_id LIMIT 2`; // --account_link_id 2 // --account_link_id 1
              ...
              var query = conn.query(myfireStr, function (err, rows) {
                ...
                let inventory_account_id = rows[1].account_id;
                let bb_account_id = rows[0].account_id;
                ...
                async.forEach(req.body.detail, function (item, callback) {
                  ...
                  queryStr = '( ' + general_journal_id + ', ' + inventory_account_id + ', ' + debitValue + ', 0, 0, 0, 1, 1, "' + tsservice.mysqlDate(req.body.create_datetime) + '", "' + tsservice.mysqlDate(req.body.create_datetime) + '", 1, 1 )';

                  queryStr += ', ( ' + general_journal_id + ', '+ bb_account_id +', 0, 0, ' + debitValue + ', 0, 1, 1, "' + tsservice.mysqlDate(req.body.create_datetime) + '", "' + tsservice.mysqlDate(req.body.create_datetime) + '", 1, 1 )';
