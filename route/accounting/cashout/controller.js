...
const controller = {
  ...
  //   REST-INSERT
  cashout_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { lastId: '', cashout_code: '', general_journal_id: '' }, label: 'Data entered successfully' };
    // validation
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        var codeData = {
          ...
          table: "cashout",
          column_id: "cashout_id",
          column_code: "cashout_code",
        }
        ...
        //   -SELECT-SPECIAL_CODE   -SELECT-CASHOUT
        var query = conn.query(myfireStr, function (err, rows) {
          ...
          if (rows[0]['code'] && rows[0]['generalledger_period_id']) {
            ...
            cashout_code = rows[0]['code'];
            data.reference_code = cashout_code;
                  ...
                  var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                    ...
                    tsservice.insertData(data, function (value) {
                      ...
                      var query = conn.query(`INSERT INTO ${CUS_DB}.general_journal` + value, function (err, rows) {
                        ...
                        continues(req, cashout_code, general_journal_id, generalledger_id, conn);
    ...
    function continues(req, cashout_code, general_journal_id, generalledger_id, conn) {
      const CUS_DB = req.body.company_db;
      ...
      tsservice.insertData(data, function (value) {
        //   -INSERT-CASHOUT
        var query = conn.query(`INSERT INTO ${CUS_DB}.cashout` + value, function (err, rows) {
          ...
          row.data.cashout_code = cashout_code;
  ...
  //   REST-UPDATE
  cashout_put: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: {}, label: 'Data updated successfully' };
    // validation
    req.assert('cashout_id', 'Journal Id is required');
    req.assert('cashout_code', 'Journal Code/No is required');
    req.assert('transaction_date', 'Transaction Date is required');
    req.assert('account_id', 'Generalledger Period Id is required');
    ...
    // update data table cashout
    var data = {
      cashout_code: req.body.cashout_code,
    ...
    var data2 = {
      reference_code: req.body.cashout_code,
      general_journal_type_id: '8', //important code
      description: 'Generated',
      transaction_date: tsservice.mysqlDate(req.body.transaction_date),
      update_by: req.body.update_by,
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        tsservice.updateData(data, function (value) {
          //   -UPDATE-CASHOUT
          var query = conn.query(`UPDATE ${CUS_DB}.cashout SET ${value} WHERE cashout_id =${req.body.cashout_id} `, function (err, rows) {
  ...
  //   REST-SELECT
  cashoutSearch_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;


    var row = { success: true, data: { cashout: [] }, label: 'Data selected successfully' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.* , t2.generalledger_period_id, t2.transaction_date, t3.department FROM ${CUS_DB}.cashout t1 INNER JOIN ${CUS_DB}.general_journal t2 ON t1.general_journal_id = t2.general_journal_id INNER JOIN ${CUS_DB}.department t3 ON t1.department_id = t3.department_id WHERE t1.is_use = 1 AND t1.is_active = 1`;
      //   -SELECT-CASHOUT   -JOIN-GENERAL_JOURNAL   -JOIN-DEPARTMENT
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
