...
module.exports = {
    generateReport: (req, callback) => {
        ...
        let employee_job_id = report_data.data.employee_job_id;
        
        req.getConnection(function (err, conn) {
            //--cmt-print: mysql cannot connect
            if (err) { row.success = false; console.log(err); row.label = 'Cannot connect to database. make sure your Database is running or contact our IT support'; res.send(row); return; }

            var myfireStr = `SELECT * FROM ${CUS_DB}.employee_job WHERE is_active = 1`;
            ...
            var query = conn.query(myfireStr, function (err, rowsAll) {
                ...
                var myfireStr = `SELECT * FROM ${CUS_DB}.employee_job WHERE employee_job_id = '${employee_job_id}' AND is_active = 1`;
