...
function select_purchase_order_code(conn, req, row) {
  const CUS_DB = req.body.company_db;
  
  return new Promise(function (resolve, reject) {

    if (req.body.purchase_order_code == "" && req.body.purchase_status_id != '1') {

      var codeData = {
        special_code_id: "PURCHASE_ORDER",
        table: "purchase_order",
        column_id: "purchase_order_id",
        column_code: "purchase_order_code",
      }
      ...
      //   -SELECT-SPECIAL_CODE   -SELECT-PURCHASE_ORDER
      var query = conn.query(myfireStr, function (err, rows) {
        ...
        if (rows[0]['code']) {
          ...
          row.data.purchase_order_code = rows[0]['code'];
          resolve(rows[0]['code']);
...
const controller = {
  //   REST-SELECT
  purchase_ordersselect_post: function (req, res, next) {

    const CUS_DB = req.body.company_db;

    var row = { success: true, data: { purchase_orders: [] }, label: 'Berhasil' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.*, t1.purchase_order_code as "code", t1.purchase_status_id as "oripurchase_status_id", t1.sub_total as "orisub_total", t1.tax as "oritax", t1.grand_total as "origrand_total", t1.downpayment as "oridownpayment", t1.landed_cost as "orilanded_cost", t1.payable as "oripayable", DATE_FORMAT(t1.transaction_date, "%d %M %Y") as "transaction_date_show" , "[]" as "purchase_order_detail",t2.businesspartner_code, t2.name as "businesspartner_name", t3.name "purchase_order_status_name", t4.name as "branch_name",t5.name "purchase_order_payment_name" , concat(t6.firstname, " ", t6.lastname) as "employee_name" FROM ${CUS_DB}.purchase_order t1 INNER JOIN ${CUS_DB}.businesspartner t2 ON t1.businesspartner_id = t2.businesspartner_id INNER JOIN ${CUS_DB}.purchase_status t3 ON t1.purchase_status_id = t3.purchase_status_id INNER JOIN ${CUS_DB}.branch t4 ON t1.branch_id = t4.branch_id INNER JOIN ${CUS_DB}.transaction_payment t5 ON t1.transaction_payment_id = t5.transaction_payment_id INNER JOIN ${CUS_DB}.employee t6 ON t1.create_by = t6.employee_id WHERE ${strwhere} t1.is_active = 1 ORDER BY t1.purchase_order_id DESC`;

      //   -SELECT-PURCHASE_ORDER   -JOIN-BUSINESSPARTNER   -JOIN-PURCHASE_STATUS   -JOIN-BRANCH   -JOIN-TRANSACTION_PAYMENT   -JOIN-EMPLOYEE
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
  ...
  //   REST-INSERT
  purchase_orders_post: function (req, res, next) {

    const CUS_DB = req.body.company_db;

    var row = { success: true, data: { purchase_order_id: '', purchase_order_code: '' }, label: 'Data entered successfully' };

    // validation
    // req.assert('purchase_order_id','PO id is required');
    // req.assert('purchase_order_code','PO Code/No is required');
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        select_purchase_order_code(conn, req, row).then(function (result) { // 1. SELECT PURCHASE ORDER CODE
          if (result) {
            purchase_order_code = result;
          }
          add_purchase_order(conn).then(function (result) { // 2. INSERT PURCHASE ORDDER

            purchase_order_id = result;
            add_purchase_order_detail(conn).then(function (result) { // 3. INSERT PURCHASE ORDER DETAIL

              row.data.purchase_order_code = purchase_order_code;
              row.data.purchase_order_id = purchase_order_id;
              ...
              conn.rollback(function () {
                console.log('false add_purchase_order_detail');
                 res.send(row); return;
              });
            });
            ...
            conn.rollback(function () {
              console.log('false add_purchase_order');
               res.send(row); return;
            });
          });
          ...
          conn.rollback(function () {
            console.log('false select_purchase_order_code');
            row = error;
             res.send(row); return;
          });
        });
    ...
    function add_purchase_order(conn) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        add_purchase_link(conn, req, row).then(function (result) {
          ...
          tsservice.insertData(data, function (value) {
            ...
            var query = conn.query(`INSERT INTO ${CUS_DB}.purchase_order` + value, function (err, rows) {
    ...
    function add_purchase_order_detail(conn) {
      return new Promise(function (resolve, reject) {
        var querystr = "";

        async.forEach(req.body.purchase_order_detail, function (item, callback) {

          if (item.is_active == 1) {
            if (querystr != "") {
              querystr += ', ';
            }
            querystr += '("' + purchase_order_id + '", "' + item.inventory_id + '", "' + item.warehouse_id + '", "' + item.row_label + '", "' + item.row_order + '", "' + item.inventory_hpp + '", "' + item.ordered + '", "' + item.orderedeqv + '", "' + item.delivered + '", "' + item.deliveredeqv + '", "' + item.price + '","' + item.landed_cost + '", "' + item.discount_persent + '", "' + item.discount_amount + '", "' + item.uom_order + '", "' + item.uom_label + '", "' + item.isdelivery + '", "' + item.tax_id + '", "' + item.tax + '", "' + item.delivery_schedule + '", "' + item.delivery_note + '", "' + item.inventory_label + '", "' + item.description + '","' + req.body.create_by + '","' + req.body.create_datetime + '","' + req.body.create_by + '","' + req.body.create_datetime + '", 1, 1)';
          }
          callback();
          ...
          if (querystr != "") {
            ...
            var myfireStr = `INSERT INTO ${CUS_DB}.purchase_order_detail( purchase_order_id, inventory_id, warehouse_id, row_label, row_order, inventory_hpp, ordered, orderedeqv, delivered, deliveredeqv, price, landed_cost, discount_persent, discount_amount, uom_order, uom_label, isdelivery, tax_id, tax, delivery_schedule, delivery_note, inventory_label, description, create_by, create_datetime, update_by, update_datetime, is_use , is_active ) VALUES ` + querystr;
  ...
  //   REST-UPDATE
  purchase_orders_put: function (req, res, next) {

    const CUS_DB = req.body.company_db;

    var row = { success: true, data: { purchase_order_id: '', purchase_order_code: '' }, label: 'Data updated successfully' };
    ...
    var purchase_order_code = req.body.purchase_order_code;
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        select_purchase_order_code(conn, req, row).then(function (result) { // 1. SELECT PURHCASE ORDER CODE
          if (result) {
            purchase_order_code = result;
            row.data.purchase_order_code = result;
          }
          edit_purchase_order(conn).then(function (result) { // 2. UPDATE PURCHASE ORDER

            edit_purchase_order_detail(conn).then(function (result) { // 3. UPDATE PURCHASE ORDER DETAIL
              ...
              conn.rollback(function () {
                console.log('false edit_purchase_order_detail');
                 res.send(row); return;
              });
            });
            ...
            conn.rollback(function () {
              console.log('false edit_purchase_order');
               res.send(row); return;
            });
          });
    ...
    function edit_purchase_order(conn) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        tsservice.updateData(data, function (value) {
          ...
          var query = conn.query(`UPDATE ${CUS_DB}.purchase_order SET ${value} WHERE purchase_order_id =${req.body.purchase_order_id} `, function (err, rows) {
    ...
    function edit_purchase_order_detail(conn) {
      return new Promise(function (resolve, reject) {
        var querystr = "";
        async.forEach(req.body.purchase_order_detail, function (item, callback) {

          if (item.purchase_order_detail_id == "" && item.is_active == 1) {

            if (querystr != "") {
              querystr += ', ';
            }
            querystr += '("' + req.body.purchase_order_id + '", "' + item.inventory_id + '", "' + item.warehouse_id + '", "' + item.row_label + '", "' + item.row_order + '", "' + item.inventory_hpp + '", "' + item.ordered + '", "' + item.orderedeqv + '", "' + item.delivered + '", "' + item.deliveredeqv + '", "' + item.price + '", "' + item.landed_cost + '", "' + item.discount_persent + '", "' + item.discount_amount + '", "' + item.uom_order + '", "' + item.uom_label + '", "' + item.isdelivery + '", "' + item.tax_id + '", "' + item.tax + '", "' + item.delivery_schedule + '", "' + item.delivery_note + '", "' + item.inventory_label + '", "' + item.description + '","' + req.body.create_by + '","' + req.body.create_datetime + '","' + req.body.create_by + '","' + req.body.create_datetime + '", 1, 1)';
            ...
            tsservice.updateData(data, function (value) {
              //   -UPDATE-PURCHASE-ORDER
              var query = conn.query(`UPDATE ${CUS_DB}.purchase_order_detail SET ${value} WHERE purchase_order_detail_id =${item.purchase_order_detail_id} `, function (err, rows) {
  ...
  //   REST-SELECT
  purchase_ordersDetail_post: function (req, res, next) {

    const CUS_DB = req.body.company_db;

    var row = { success: true, data: { purchase_order_detail: [] }, label: 'Data entered successfully' };
    // validation
    req.assert('purchase_order_id', 'PO id is required');
    ...
    queryOption = "AND t1.purchase_order_id = '" + req.body.purchase_order_id + "'";
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.*, t2.*, t2.selling_price as "oriprice", 0 as "min_price" FROM ${CUS_DB}.purchase_order_detail t1 INNER JOIN ${CUS_DB}.inventory t2 ON t1.inventory_id = t2.inventory_id WHERE t1.is_use = 1 AND t1.is_active = 1 ${queryOption}`;

      //   -SELECT-PURCHASE-ORDER-DETAIL   -JOIN-INVENTORY
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
        ...
        row.data.purchase_order_detail = rows;
         res.send(row); return;
      });
