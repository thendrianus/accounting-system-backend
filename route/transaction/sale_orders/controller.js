...
function select_sale_order_code(conn, req, row) {
  const CUS_DB = req.body.company_db;
  return new Promise(function (resolve, reject) {

    if (req.body.sale_order_code == "" && req.body.sale_status_id != '1') {

      var codeData = {
        special_code_id: "SALE_ORDER",
        table: "sale_order",
        column_id: "sale_order_id",
        column_code: "sale_order_code",
      }
      ...
      //   -SELECT-SPECIAL_CODE   -SELECT-SALE_ORDER
      var query = conn.query(myfireStr, function (err, rows) {
        ...
        if (rows[0]['code']) {
          ...
          row.data.sale_order_code = rows[0]['code'];
          resolve(rows[0]['code']);
...
const controller = {
  //   REST-SELECT
  sale_ordersselect_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    var row = { success: true, data: { sale_orders: [] }, label: 'Berhasil' };
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.*, t1.sale_order_code as "code", t1.sale_status_id as "orisale_status_id", t1.sub_total as "orisub_total", t1.tax as "oritax", t1.grand_total as "origrand_total", t1.downpayment as "oridownpayment", t1.landed_cost as "orilanded_cost", t1.receivable as "orireceivable", DATE_FORMAT(t1.transaction_date, "%d %M %Y") as "transaction_date_show" , "[]" as "sale_order_detail",t2.businesspartner_code, t2.name as "businesspartner_name", t3.name "sale_status_name", t4.name as "branch_name",t5.name "sale_order_payment_name" , concat(t6.firstname, " ", t6.lastname) as "employee_name" FROM ${CUS_DB}.sale_order t1 INNER JOIN ${CUS_DB}.businesspartner t2 ON t1.businesspartner_id = t2.businesspartner_id INNER JOIN ${CUS_DB}.sale_status t3 ON t1.sale_status_id = t3.sale_status_id INNER JOIN ${CUS_DB}.branch t4 ON t1.branch_id = t4.branch_id INNER JOIN ${CUS_DB}.transaction_payment t5 ON t1.transaction_payment_id = t5.transaction_payment_id INNER JOIN ${CUS_DB}.employee t6 ON t1.create_by = t6.employee_id WHERE ${strwhere} t1.is_active = 1 ORDER BY t1.sale_order_id DESC`;

      //   -SELECT-SALE_ORDER   -JOIN-BUSINESSPARTNER   -JOIN-SALE_STATUS   -JOIN-BRANCH   -JOIN-TRANSACTION_PAYMENT   -JOIN-EMPLOYEE
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
  ...
  //   REST-INSERT
  sale_orders_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    var row = { success: true, data: { sale_order_id: '', sale_order_code: '' }, label: 'Data entered successfully' };

    // validation
    // req.assert('sale_order_id','Sale Order Id is required');
    // req.assert('sale_order_code','Sale Order Code/Id is required');
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        select_sale_order_code(conn, req, row).then(function (result) { // 1. SELECT SALE ORDER CODE
          if (result) {
            sale_order_code = result;
          }
          add_sale_order(conn).then(function (result) { // 2. INSERT SALE ORDER

            sale_order_id = result;
            add_sale_order_detail(conn).then(function (result) { // 3. INSERT SALE ORDER DETAIL

              row.data.sale_order_code = sale_order_code;
              row.data.sale_order_id = sale_order_id;
              ...
              conn.rollback(function () {
                console.log('false add_sale_order_detail');
                 res.send(row); return;
              });
            });
            ...
            conn.rollback(function () {
              console.log('false add_sale_order');
               res.send(row); return;
            });
          });
          ...
          conn.rollback(function () {
            console.log('false select_sale_order_code');
             res.send(row); return;
          });
        });
      });
      ...
      return new Promise(function (resolve, reject) {
        ...
        add_sale_link(conn, req, row).then(function (result) {
          ...
          tsservice.insertData(data, function (value) {
            //   -INSERT-SALE_ORDER
            var query = conn.query(`INSERT INTO ${CUS_DB}.sale_order` + value, function (err, rows) {
    ...
    function add_sale_order_detail(conn) {
      return new Promise(function (resolve, reject) {
        var querystr = "";

        async.forEach(req.body.sale_order_detail, function (item, callback) {

          if (item.is_active == 1) {
            if (querystr != "") {
              querystr += ', ';
            }
            querystr += '("' + sale_order_id + '", "' + item.inventory_id + '", "' + item.warehouse_id + '", "' + item.row_label + '", "' + item.row_order + '", "' + item.inventory_hpp + '", "' + item.ordered + '", "' + item.orderedeqv + '", "' + item.delivered + '", "' + item.deliveredeqv + '", "' + item.price + '","' + item.landed_cost + '", "' + item.discount_persent + '", "' + item.discount_amount + '", "' + item.uom_order + '", "' + item.uom_label + '", "' + item.isdelivery + '", "' + item.tax_id + '", "' + item.tax + '", "' + item.delivery_schedule + '", "' + item.delivery_note + '", "' + item.inventory_label + '","' + item.description + '","' + req.body.create_by + '","' + req.body.create_datetime + '","' + req.body.create_by + '","' + req.body.create_datetime + '", 1, 1)';
          }
          callback();
          ...
          if (querystr != "") {
            ...
            var myfireStr = `INSERT INTO ${CUS_DB}.sale_order_detail( sale_order_id, inventory_id, warehouse_id, row_label, row_order, inventory_hpp, ordered, orderedeqv, delivered, deliveredeqv, price, landed_cost, discount_persent, discount_amount, uom_order, uom_label, isdelivery, tax_id, tax, delivery_schedule, delivery_note, inventory_label, description, create_by, create_datetime, update_by, update_datetime, is_use , is_active ) VALUES ` + querystr;
  ...
  //   REST-UPDATE
  sale_orders_put: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    var row = { success: true, data: { sale_order_id: '', sale_order_code: '' }, label: 'Data updated successfully' };
    ...
    var sale_order_code = req.body.sale_order_code;
    var sale_order_id = req.body.sale_order_id;
    ...
    req.getConnection(function (err, conn) {
      ...
      conn.beginTransaction(function (err) {
        ...
        select_sale_order_code(conn, req, row).then(function (result) { // 1.SELECT SALE ORDER
          if (result) {
            sale_order_code = result;
            row.data.sale_order_code = result;
          }
          edit_sale_order(conn).then(function (result) { // 2. UPDATE SALE ORDER

            edit_sale_order_detail(conn).then(function (result) { // 3. UPDATE SALE ORDER DETAIL
              ...
              conn.rollback(function () {
                console.log('false edit_sale_order_detail');
                 res.send(row); return;
              });
            });
            ...
            conn.rollback(function () {
              console.log('false edit_sale_order');
               res.send(row); return;
            });
          });
    ...
    function edit_sale_order(conn) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        tsservice.updateData(data, function (value) {
          //   -UPDATE-SALE_ORDER
          var query = conn.query(`UPDATE ${CUS_DB}.sale_order SET ${value} WHERE sale_order_id =${req.body.sale_order_id} `, function (err, rows) {
    ...
    function edit_sale_order_detail(conn) {
      return new Promise(function (resolve, reject) {
        var querystr = "";
        async.forEach(req.body.sale_order_detail, function (item, callback) {

          if (item.sale_order_detail_id == "" && item.is_active == 1) {

            if (querystr != "") {
              querystr += ', ';
            }
            querystr += '("' + req.body.sale_order_id + '", "' + item.inventory_id + '", "' + item.warehouse_id + '", "' + item.row_label + '", "' + item.row_order + '", "' + item.inventory_hpp + '", "' + item.ordered + '", "' + item.orderedeqv + '", "' + item.delivered + '", "' + item.deliveredeqv + '", "' + item.price + '", "' + item.landed_cost + '", "' + item.discount_persent + '", "' + item.discount_amount + '", "' + item.uom_order + '", "' + item.uom_label + '", "' + item.isdelivery + '", "' + item.tax_id + '", "' + item.tax + '", "' + item.delivery_schedule + '", "' + item.delivery_note + '", "' + item.inventory_label + '", "' + item.description + '","' + req.body.create_by + '","' + req.body.create_datetime + '","' + req.body.create_by + '","' + req.body.create_datetime + '", 1, 1)';
            ...
            tsservice.updateData(data, function (value) {
              ...
              var query = conn.query(`UPDATE ${CUS_DB}.sale_order_detail SET ${value} WHERE sale_order_detail_id =${item.sale_order_detail_id} `, function (err, rows) {
  ...
  //   REST-SELECT
  sale_ordersDetail_post: function (req, res, next) {
    const CUS_DB = req.body.company_db;
    var row = { success: true, data: { sale_order_detail: [] }, label: 'Data selected successfully' };
    // validation
    req.assert('sale_order_id', 'Sale Order Id is required');
    ...
    queryOption = "AND t1.sale_order_id = '" + req.body.sale_order_id + "'";
    ...
    req.getConnection(function (err, conn) {
      ...
      var myfireStr = `SELECT t1.*, t2.*, t2.selling_price as "oriprice", 0 as "min_price" FROM ${CUS_DB}.sale_order_detail t1 INNER JOIN ${CUS_DB}.inventory t2 ON t1.inventory_id = t2.inventory_id WHERE t1.is_use = 1 AND t1.is_active = 1 ${queryOption}`;

      //   -SELECT-SALE_ORDER   -JOIN-INVENTORY
      var query = conn.query(myfireStr, function (err, rows) {
        if (err) {
          row.success = false; console.log(err);
          row.label = 'Failed to select database. make sure your Database is running or contact our IT support';
        ...
        row.data.sale_order_detail = rows;
         res.send(row); return;
      });
